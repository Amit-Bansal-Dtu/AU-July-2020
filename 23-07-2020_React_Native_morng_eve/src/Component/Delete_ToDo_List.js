...
const Delete_ToDo=({navigation})=>{
    ...
    const Delete=async()=>{
        console.log(key);
        let array = JSON.parse(await AsyncStorage.getItem("ToDoList"));
        let idx;
        array.forEach((val, index)=>{if(val==key){idx=index}});
        array.splice(idx, 1);
        await AsyncStorage.setItem("ToDoList",JSON.stringify(array));
    ...
    return(
        ...
        <View  style={{flex:1}}>
            ...
            <TouchableOpacity style={Styles.create} onPress={Delete}><Text style={Styles.text}>Delete Selected To Do</Text></TouchableOpacity>
